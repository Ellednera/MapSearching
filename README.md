...
### Conventions Used
In order to make the documentation easy to read, the following conventions will be used

| Symbol  | Meaning| Comments |
| :-----: | :-----: | :------ |
| + | public method | - |
| - | private method | - |
| ! | deprecated | This can appear with other symbols eg !+ means that a public method is deprecated |
| : | return type | Anything after this symbol is the return type |
| c | constructor | c\[+/-] |
| s | static method | s\[+/-] |
| i | inner class | i\[+/-] |

! preceeds all symbols while alphabets preceed +- to avoid confusion

Example: a deprecated public static method named eat will be written as *!s+ eat : return value*
...
#### c Station(int x, int y, String name)
...
#### + connectTo(Station s, int distance) : void
...
#### + isConnectedTo(Station s) : boolean
Checks if this current station is connected to another *Station* s

#### + distanceFrom(Station s) : int
The distance from this current station to another *Station* s

#### + verbose_distanceFrom(Station s) : void
More text :)

#### + allConnections() : Map<Station, Integer>
...
#### + createBridges(Station s1, Station s2) : void
...
#### + processFullNetwork(ArrayList<List\<Station>> \_partialNetworks, int maxLayers, boolean verbose) : ArrayList<List\<Station>>
...
#### - linkAllConnectingStations(ArrayList<List\<Station>> \_partialNetworks, boolean verbose) : ArrayList<List\<Station>>
...
#### + showAvailablePaths(String start_name, String destination_name, ArrayList<List\<Station>> processedPaths, boolean verbose, int capacity, boolean analyseCapacity) : ArrayList<List\<Station>>
...
### 3. assets::GPS
#### + findChosenPath(ArrayList<java.util.List\<Station>> paths_2_filter, String check_start_name, String check_destination_name) : Station[]
...
#### + findChosenPath(ArrayList<java.util.List\<Station>> paths_2_filter, ArrayList\<Station> stationNames, int capacity) : Station[]
...
#### - schwartzianTransform(ArrayList<java.util.List\<Station>> paths_2_filter) : Station[]
Schwartzian transformation used by *findChosenPath* (COP) method

#### - correctedDirection(Station[] shortestPath, String check_start_name, String check_destination_name) : Station[]
...
#### - renderRegisterRouteButton(Button registerRouteButton, Button clearRouteButton) : Panel
...
#### - renderRouteDisplay(TextField routeTF) : Panel
...
#### - renderRouteButton(Button routeButton) : Panel
...
#### + getSelectedStations() : ArrayList\<Station>
...
#### s- test_stationDictionary(CityMap map, String lookup_name) : void
#### s- test_stationDictionary(CityMap map, String[] lookup_names) : void
